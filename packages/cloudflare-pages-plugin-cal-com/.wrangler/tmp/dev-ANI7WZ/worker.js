// .wrangler/tmp/bundle-mZH5TZ/checked-fetch.js
...
var post_request_body = z.object({
  triggerEvent: z.enum([
    "BOOKING_CREATED",
    "BOOKING_CANCELLED",
    "BOOKING_RESCHEDULED",
...
var hexStringToArrayBuffer = (hex) => {
...
var defaultOrOptional = (default_value, optional_value) => {
  if (optional_value !== void 0) {
    return optional_value;
  } else {
    return default_value;
  }
};
var badRequest = (details) => {
...
var PREFIX2 = `[${"\u{1F5E3}\uFE0F"} cal.com webhooks plugin]`;
var defaults = {
  secret: void 0,
  shouldValidate: true
};
var calComPlugin = (pluginArgs2) => {
  const options = pluginArgs2 || {};
  ...
  console.log({
    message: `${PREFIX2} configuration`,
    secret,
    shouldValidate
  });
  let key;
  const decoder = new TextDecoder("utf-8");
  return async function calComPluginInner(ctx) {
    const audit_trail = [];
    if (secret) {
    ...
    if (!secret) {
      throw new Error(
        `${PREFIX2} secret not set. Set a signing secret for your cal.com webhooks either passing it when you instantiate this plugin, or using the environment variable CAL_WEBHOOK_SECRET`
      );
    }
    if (!key) {
    ...
    console.log({ message: `${PREFIX2} request audit trail`, audit_trail });
    return ctx.next();
  };
};
...
// .wrangler/tmp/bundle-mZH5TZ/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-mZH5TZ/middleware-loader.entry.ts
